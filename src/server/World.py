...
class World(object):
    ...
    def move_object(self, obj, to_x,to_y):
        self.data[obj.x,obj.y].remove(obj)
        self.data[to_x,to_y].append(obj)
        print(self.data)

    def update_world(self, actions):
        random.shuffle(actions)
        for action in actions:
            where,entity_symbol,what,parameter = action
            x,y = where
            entity = self.get_entity(x,y,entity_symbol)
            entity.action[what](parameter)
